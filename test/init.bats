...
@test "setup shell completions" {
  ...
  run tfenv-init - bash
  assert_success
  assert_line "source '${root}/test/../libexec/../completions/tfenv.bash'"
}
...
@test "setup shell completions (zsh)" {
  root="$(cd $BATS_TEST_DIRNAME/.. && pwd)"
  run tfenv-init - zsh
  assert_success
  assert_line "source '${root}/test/../libexec/../completions/tfenv.zsh'"
}
...
@test "setup shell completions (fish)" {
  ...
  run tfenv-init - fish
  assert_success
  assert_line "source '${root}/test/../libexec/../completions/tfenv.fish'"
}

@test "fish instructions" {
  run tfenv-init fish
  assert [ "$status" -eq 1 ]
  assert_line 'status --is-interactive; and source (tfenv init -|psub)'
}
